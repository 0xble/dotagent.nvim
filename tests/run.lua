...
assert(
  command_response.items[1].detail == nil,
  "completion items should not repeat the description in detail"
)
assert(
  command_response.items[1].documentation.kind == vim.lsp.protocol.MarkupKind.Markdown,
  "documentation should render as markdown"
)
assert(
  command_response.items[1].documentation.value:find('description: "Ship the current branch"', 1, true)
    ~= nil,
  "documentation should keep the quoted description line"
)
assert(
  command_response.items[1].documentation.value:find("Path: " .. legacy_commands .. "/ship.md", 1, true)
    ~= nil,
  "documentation should include the item path"
)
assert(
  command_response.items[1].documentation.value:find("# Ship", 1, true) ~= nil,
  "documentation should include the file body without frontmatter"
)
assert(
  not command_response.items[1].documentation.value:find("\n---\n", 1, true),
